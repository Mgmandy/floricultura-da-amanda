...
$.ajax(url)
    ...
    .done(function(xml){
            $(xml).find("flor").each(function(){
                ...
                if(id == pos){
                $("#individual").append('<div class="card"><p class="flores">Procurado</p> <img class="foto"src="'+ $(this).find("id").text() +'.jpg"><p class="vivo">disonivel ou esgotado</p> <p class="nome">'+ $(this).find("nome").text() +'</p> <p class="rec"> <img class="berrys" src="Berrys.png">'+ $(this).find("recompensa").text()+' </p><p> Status:'+ $(this).find("status").text()+' </p><\div>');
                }
            });
